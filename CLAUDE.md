...
- **Restaurant list screen (`ListFragment`) does not show the number of interested colleagues per restaurant.** `Restaurant.java` and `RestaurantRepository` currently have no notion of per-restaurant colleague interest counts (only `RestaurantInterest`/`setUserChoice` for the single-restaurant detail screen). Showing this on the list requires a separate repository refactor (e.g. joining restaurant data with interest counts across all nearby restaurants) — do this as its own follow-up task, not bundled into the list UI work.
